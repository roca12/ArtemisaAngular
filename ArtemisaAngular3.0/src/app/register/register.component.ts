...
export class RegisterComponent {
  ...
  register(){
    const { username, email, password, passwordConfirm, recaptcha } = this.registerForm.value;
    ...
    this.mailService.enviarCodigo(email, username).subscribe({
      next: (response) => {
        ...
        modalRef.result.then((result) => {
          ...
          if(error.status === 409){
          this.toastr.warning('El nombre de usuario o correo electrónico ya está en uso.', 'Aviso');
      ...
      error: (error) => {
        if(error.status === 409){
          this.toastr.error('El nombre de usuario o correo electrónico ya está en uso.', 'Error');
          console.error(error);
        }else{
